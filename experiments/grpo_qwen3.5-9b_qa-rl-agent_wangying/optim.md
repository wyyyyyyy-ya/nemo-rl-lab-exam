...
| 调优项 | 预期效果 |
|--------|----------|
| Prompt 防幻觉 | 减少 MRB/控制图式「假检索」 |
| Fallback + 短 query | 减少 0 命中（安全卫生、MRB） |
| 过滤片段 + 中性 search | 控制图类更易命中 Exclude 相关正文 |
| 综合 | accuracy **68% → 70%+**（不保证，需 submit 后验证） |
...
| 禁止幻觉 Prompt | — | 部分 | ✅ |
| 检索 `.txt` | — | — | ✅ |
| `search_top_k` / `snippet_chars` | 3 / 400 | 3 / 400 | 5 / 600 |
| `max_num_steps` | 400 | 400 | 250 |
...
- 检索未命中时，对 `\boxed{}` 做软惩罚（当前仅 Prompt 约束，不硬拦）  
- BM25 / 向量检索替代简单 grep  
